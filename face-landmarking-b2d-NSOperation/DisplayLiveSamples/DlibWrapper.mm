...
- (void)doWorkOnSampleBuffer:(CMSampleBufferRef)sampleBuffer inRects:(NSArray<NSValue *> *)rects {
    ...
    NSOperation *b2d1 = [NSBlockOperation blockOperationWithBlock:^{
        for(int i=0;i<height/2;i++){
            for(int j=0;j<width;j++){
                
                // assuming bgra format here
                //long bufferLocation = position * 4; //(row * width + column) * 4;
                long bufferLocation = (i*width+j)*4;
    ...
    [q addOperation:b2d1];
    [b2d1 completionBlock];


    NSOperation *b2d2 = [NSBlockOperation blockOperationWithBlock:^{
        for(unsigned long i=height/2;i<height;i++){
            for(int j=0;j<width;j++){
